...
import { getUsers, createUser, updateUser, toggleUserStatus } from '@/api/admin'
...
const handleToggleStatus = async (row) => {
  try {
    ...
    await ElMessageBox.confirm(
      `确定要${statusText}员工「${row.real_name}」吗？`,
      `${statusText}确认`,
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    await toggleUserStatus(row.id)
    ElMessage.success(`员工「${row.real_name}」已${statusText}`)
    fetchUsers()
  } catch {
    // 用户取消或接口报错已处理
  }
  ...
  <div class="users-page">
    ...
    <div class="table-container">
      <el-table :data="users" v-loading="loading" class="users-table">
        ...
        <el-table-column prop="real_name" label="姓名" min-width="160" align="center">
          <template #default="{ row }">
